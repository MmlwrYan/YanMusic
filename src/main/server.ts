...
export async function initApiServer(): Promise<void> {
  ...
  // 将 server/node_modules 注入到 Node.js 模块搜索路径，
  // 确保 server/util/*.js 内部的 require('crypto-js') 等在 asar 打包后仍能正确解析。
  if (!module.paths.includes(serverNodeModulesPath)) {
    module.paths.unshift(serverNodeModulesPath);
  }
