...
export const applyKugouApiNetworkSettings = (
  ...
  // YanMusic 保留行为：酷狗 API 专用代理（kugouApiProxyUrl）→ 覆盖 Kugou API 分区的
  // 代理规则；未配置时回退到全局网络策略（updateNetworkPolicy 统一重放）。
  const kugouProxyUrl = settings.kugouApiProxyUrl.trim();
  if (kugouProxyUrl) {
    void session
      .fromPartition(KUGOU_API_SESSION_PARTITION)
      .setProxy({ mode: 'fixed_servers', proxyRules: kugouProxyUrl })
      .catch((error: unknown) => {
  ...
  try {
    ...
    log.info('[Network] Applied Kugou API network settings', {
      ...
      kugouProxyOverride: kugouProxyUrl ? 'configured' : 'global',
