...
export function loadSpectrumCapture(): NativeSpectrumCapture | null {
  ...
  try {
    const resourcePath = app.isPackaged
      ? path.join(process.resourcesPath, 'native', 'yan-spectrum-capture.node')
      : path.join(__dirname, '../../native/yan-spectrum-capture/yan-spectrum-capture.node');
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      nativeModule = require('../../native/yan-spectrum-capture') as NativeSpectrumCapture;
      return nativeModule;
    } catch (err2) {
      log.warn(
