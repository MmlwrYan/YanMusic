...
const getPluginCompatibilityError = (plugin: EchoPluginDescriptor) =>
  plugin.compatibility.compatible
    ? ''
    : plugin.compatibility.message || '插件与当前 YanMusic 主程序版本不兼容';
...
const normalizeMarketplaceStatsApiUrl = (value?: string) =>
  String(
    value || process.env.yanmusic_PLUGIN_STATS_API_URL || DEFAULT_PLUGIN_MARKETPLACE_STATS_API_URL,
  )
    .trim()
    .replace(/\/+$/, '');
    ...
    .map((plugin) => {
      ...
      return {
        ...
        updateAvailable: installed
          ...
          : false,
        compatibility: getyanmusicCompatibility(plugin.manifest),
...
const findManifestDirectoryCandidate = (directory: string) => {
  if (!existsSync(directory) || !statSync(directory).isDirectory()) return '';
  if (existsSync(join(directory, PLUGIN_MANIFEST_FILE))) return directory;
...
const findPluginInstallSourceDirectory = (extractDirectory: string, packagePath: string) => {
  ...
  if (!isSafeMarketplacePackagePath(normalizedPackagePath)) throw new Error('插件包路径非法');
  ...
  if (normalizedPackagePath) {
    ...
    const matched = findManifestDirectoryCandidate(candidate);
    if (matched) return matched;
  }

  const rootMatched = findManifestDirectoryCandidate(archiveRoot);
...
const installPluginDirectory = async (
  ...
  const stagingParent = mkdtempSync(join(tmpdir(), 'yan-plugin-install-'));
  ...
  try {
    ...
    if (!descriptor.compatibility.compatible) {
      throw new Error(descriptor.compatibility.message || '插件与当前 YanMusic 版本不兼容');
    }
...
export const installPluginFromMarketplace = async (
  ...
  try {
    ...
    if (!plugin.compatibility.compatible) {
      ...
      return {
        ok: false,
        error: plugin.compatibility.message || '插件与当前 YanMusic 版本不兼容',
      };
    }
    installTarget = plugin;

    const tempDirectory = mkdtempSync(join(tmpdir(), 'yan-plugin-download-'));
    try {
...
const installPluginFromLocalSource = async (
  ...
  try {
    ...
    const tempDirectory = mkdtempSync(join(tmpdir(), 'yan-plugin-local-'));
    try {
      const extractDirectory = join(tempDirectory, 'extracted');
      mkdirSync(extractDirectory, { recursive: true });
      await runWithTimeout(
        extractZipWithStreamZip(source.path, extractDirectory),
        PLUGIN_MARKETPLACE_EXTRACT_TIMEOUT_MS,
        '插件安装包解压超时',
        'node-stream-zip',
      );
...
const buildPluginProcessEnv = (
  ...
  env.yanmusic_PLUGIN_ID = plugin.id;
  env.yanmusic_PLUGIN_DIR = pluginRoot;
