...
const getWindowsShortcutCandidates = (kind: 'desktop' | 'taskbar') => {
  ...
  const names = Array.from(new Set([app.getName(), 'YanMusic', 'yan-music'].filter(Boolean)));
  const directory =
    kind === 'desktop'
      ? app.getPath('desktop')
      : join(
...
const isYanMusicDesktopEntry = (content: string) => {
  const lower = content.toLowerCase();
  return (
    /^Name\s*=\s*YanMusic\s*$/im.test(content) ||
    lower.includes('yanmusic') ||
...
const applyLinuxDesktopEntryIcon = (iconPath: string) => {
  const desktopEntry = getLinuxDesktopEntryCandidates().find((filePath) => {
    try {
      return isYanMusicDesktopEntry(readFileSync(filePath, 'utf8'));
    } catch {
      return false;
    }
  });
