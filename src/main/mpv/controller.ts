...
export class MpvController extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    if (!this.addon) throw new Error('Failed to load yan-mpv-player addon');
  ...
  private loadAddon(): MpvAddon | null {
    try {
      const addonPath = app.isPackaged
        ? path.join(process.resourcesPath, 'native', 'yan-mpv-player.node')
        : path.join(__dirname, '../../native/yan-mpv-player/yan-mpv-player.node');
      ...
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        return require('../../native/yan-mpv-player') as MpvAddon;
      } catch {
        log.error('[MpvController] Failed to load yan-mpv-player addon');
        return null;
      }
    }
  }
