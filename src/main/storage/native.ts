...
const loadNativeStorageAddon = (): NativeStorageAddon => {
  ...
  const primaryPath = app.isPackaged
    ? join(process.resourcesPath, 'native', 'yan-storage.node')
    : join(__dirname, '../../native/yan-storage/yan-storage.node');
  ...
  try {
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      addon = require('../../native/yan-storage') as NativeStorageAddon;
      return addon;
    } catch (fallbackError) {
...
export const getNativeStorage = (): NativeStorageAddon => {
  ...
  if (!initialized) {
    ...
    nativeStorage.initialize(join(userDataPath, 'YanMusic.sqlite'));
    initialized = true;
  }
