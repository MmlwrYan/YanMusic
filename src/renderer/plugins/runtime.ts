...
const applySurfaceContributions = () => {
  ...
  body.classList.toggle('yan-surface-translucent', enabledContributions.length > 0);
...
const applyPageTransitionContributions = () => {
  ...
  body.classList.toggle('yan-page-transition-customized', contributions.length > 0);
...
const insertMountContainer = (
  ...
  if (position === 'replace') {
    ...
    target.setAttribute('data-yan-plugin-replaced', 'true');
    return;
  }
...
const createMountedComponentDisposer = (
  ...
  container.className = ['yan-plugin-mount', options.className].filter(Boolean).join(' ');
  ...
  return () => {
    ...
    // 清理 DOM
    if (position === 'replace') {
      const replaced = container.querySelector<HTMLElement>('[data-yan-plugin-replaced="true"]');
      if (replaced) {
        replaced.removeAttribute('data-yan-plugin-replaced');
...
const SCROLL_CONTAINER_SELECTOR = '[data-yan-scroll-container]';
        ...
        () =>
          addDisposable(
            createMountedComponentDisposer(pluginId, host, document.body, component, {
              ...options,
              position: 'append',
              className: ['yan-plugin-teleport', options?.className].filter(Boolean).join(' '),
            }),
          ),
        () => undefined,
      ),
...
const extractPluginIdFromErrorSource = (...sources: unknown[]) => {
  const text = sources
    ...
    .join('\n');
  return text.match(/yan-plugin:([a-zA-Z0-9._-]+)/)?.[1] ?? '';
};
...
function createStyleDisposer(pluginId: string, cssText: string, styleId = 'runtime') {
  const elementId = `yan-plugin-style-${pluginId}-${styleId}`;
...
const importPluginModule = async (descriptor: EchoPluginDescriptor, code: string) => {
  const blob = new Blob([`${code}\n//# sourceURL=yan-plugin:${descriptor.id}`], {
 ...
 */
const refreshCurrentPlaybackState = () => {
  try {
    const playerStore = usePlayerStore();
    const lyricStore = useLyricStore();
