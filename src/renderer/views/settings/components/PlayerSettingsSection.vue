...
const updateCacheSecs = (value: string | number) => {
  settingStore.audioCacheSecs = clampNumber(value, 30, 0, MAX_CACHE_SECS);
};
...
const updateDemuxerMaxBytes = (value: string | number) => {
  settingStore.audioDemuxerMaxMB = clampNumber(value, 48, 0, MAX_DEMUXER_CACHE_MB);
};

const updateDemuxerMaxBackBytes = (value: string | number) => {
  settingStore.audioDemuxerBackMB = clampNumber(value, 12, 0, MAX_DEMUXER_CACHE_MB);
};

const updateAudioBufferSecs = (value: string | number) => {
  settingStore.audioBufferSecs = clampNumber(value, 0.5, 0, MAX_AUDIO_BUFFER_SECS);
};
  ...
  <SettingsSectionShell id="player" :title="sectionTitles.player.label">
    ...
    <div class="settings-item">
      <div class="space-y-1">
        ...
        <p class="text-sm text-text-secondary">网络预读缓存的最大时长，默认 30 秒</p>
      </div>
      <InputNumber
        class="w-45"
