...
<template>
  <Popover
    ...
    :side-offset="6"
    :show-arrow="false"
    content-class="yan-select-content"
  >
    <template #trigger>
      <div
        :class="['yan-select-trigger', props.class]"
        :data-state="open ? 'open' : 'closed'"
        @mouseenter="isHovered = true"
        @mouseleave="isHovered = false"
      >
        <!-- 多选标签 -->
        <div v-if="props.multiple" class="yan-select-tags">
          <span v-for="tag in visibleTags" :key="String(tag.value)" class="yan-select-tag">
            <span class="yan-select-tag-text">{{ tag.label }}</span>
            <span class="yan-select-tag-close" @click="removeTag(tag.value, $event)">
              <Icon :icon="iconX" width="10" height="10" />
            </span>
          </span>
          <span v-if="overflowCount > 0" class="yan-select-tag yan-select-tag--count">
            +{{ overflowCount }}
          </span>
          <input
            ...
            ref="inputRef"
            v-model="searchTerm"
            class="yan-select-input"
            :placeholder="selectedTags.length === 0 ? props.placeholder : ''"
            @keydown.stop
          />
          <span v-else-if="selectedTags.length === 0" class="yan-select-placeholder">
            {{ props.placeholder }}
          </span>
        </div>
        <!-- 单选 -->
        <template v-else>
          <input
            ...
            @keydown.stop
          />
          <span v-else class="yan-select-value" :class="{ 'is-placeholder': !selectedLabel }">
            {{ selectedLabel || props.placeholder }}
          </span>
        </template>
        <!-- 清除 / 箭头 -->
        <span v-if="showClear" class="yan-select-clear" @click="handleClear">
          <Icon :icon="iconX" width="12" height="12" />
        </span>
        <span v-else class="yan-select-arrow" :class="{ 'is-open': open }">
          <Icon :icon="iconChevronDown" width="14" height="14" />
        </span>
      </div>
    </template>

    <div v-if="filteredOptions.length === 0" class="yan-select-empty">无匹配项</div>
    <div v-else class="yan-select-list" @scroll.passive="handleScroll">
      <div :style="useVirtual ? { height: totalHeight + 'px', position: 'relative' } : {}">
        <div :style="useVirtual ? { transform: `translateY(${offsetY}px)` } : {}">
          ...
          >
            <span class="yan-select-item-text">{{ option.label }}</span>
            <span v-if="isSelected(option.value)" class="yan-select-item-check">✓</span>
          </button>
        </div>
      </div>
    </div>
...
.yan-select-value.is-placeholder {
  @apply text-text-secondary/70;
}

.yan-select-placeholder {
...
.yan-select-item:hover {
  background: var(--row-hover-bg);
}

.yan-select-item.is-selected {
  color: var(--color-primary);
  background: var(--row-selected-bg);
}

.yan-select-item.is-disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

.yan-select-item.is-disabled:hover {
  background: transparent;
}

.yan-select-item-text {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.yan-select-item-check {
  color: var(--color-primary);
  font-size: 14px;
  font-weight: 700;
  flex-shrink: 0;
