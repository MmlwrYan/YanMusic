...
import loader2Icon from '@iconify/icons-tabler/loader-2';
import uploadIcon from '@iconify/icons-tabler/upload';
import { iconCheckMark, iconSearch, iconX } from '@/icons';
import { search } from '@/api/search';
import { uploadToCloud } from '@/api/cloud';
import {
...
/** 云盘上传支持的音频扩展名（与后端接收范围一致） */
const CLOUD_UPLOAD_EXTENSIONS = [
  'aac',
  'aif',
  'aiff',
  'ape',
...
const CLOUD_UPLOAD_MAX_SIZE = 100 * 1024 * 1024;
/** 文件夹模式下收集文件数量上限 */
const CLOUD_UPLOAD_FOLDER_LIMIT = 10000;
...
/** path -> 本地文件对象：上传阶段按需读取二进制，避免一次性占用内存 */
...
watch(open, (v) => {
  if (!v) {
    ...
    if (step.value === 'uploading' && cloudUploadStore.status === 'running') {
      if (cloudUploadStore.backgroundConfirmDismissed) {
    ...
    // 非上传中关闭：后台任务运行期间不清本地文件引用，避免打断正在进行的读取
...
const confirmBackgroundUpload = () => {
  ...
  if (neverShowBackgroundConfirm.value) {
    cloudUploadStore.backgroundConfirmDismissed = true;
  }
  enterBackgroundMode();
...
const isUploadAudioName = (name: string) => {
  ...
  return CLOUD_UPLOAD_EXTENSIONS.includes(extension);
};

const resolveTitleAndArtist = (fileName: string) => {
  const baseName = fileName.replace(/\.[^.]+$/, '').trim();
  const segments = baseName.split(/\s+-\s+/);
  if (segments.length >= 2) {
    const artist = segments.shift()?.trim() || '';
    const title = segments.join(' - ').trim() || baseName;
    if (artist && title) return { title, artist };
  }
  return { title: baseName || fileName, artist: '' };
};

const registerPickedFile = (file: File, mode: PickMode) => {
  const relativePath = (file as File & { webkitRelativePath?: string }).webkitRelativePath || '';
  const basePath = mode === 'folder' && relativePath ? relativePath : file.name;
  const path = pickedFiles.has(basePath) ? `${basePath}#${++pathSeq}` : basePath;
  pickedFiles.set(path, file);
  const { title, artist } = resolveTitleAndArtist(file.name);
  return {
    name: file.name,
    path,
    title,
    artist,
    size: file.size,
    extension: `.${normalizeExtension(file.name.split('.').pop() || '')}`,
    modifiedAt: file.lastModified || 0,
    status: 'pending' as const,
    matchStatus: 'pending' as const,
  } satisfies CloudUploadItem;
};

const handlePickedFiles = async (files: File[], mode: PickMode) => {
  if (files.length === 0) return;
  const errors: string[] = [];
  let selected = files.filter((file) => {
    if (!isUploadAudioName(file.name)) {
      errors.push(`${file.name}: 不是支持的音频文件`);
      return false;
    }
    if (file.size <= 0 || file.size > CLOUD_UPLOAD_MAX_SIZE) {
      errors.push(`${file.name}: 文件为空或超过 ${CLOUD_UPLOAD_MAX_SIZE / 1024 / 1024}MB 限制`);
      return false;
    }
    return true;
  });

  if (selected.length > CLOUD_UPLOAD_FOLDER_LIMIT) {
    errors.push(`文件数量超过 ${CLOUD_UPLOAD_FOLDER_LIMIT} 个上限，已截断`);
    selected = selected.slice(0, CLOUD_UPLOAD_FOLDER_LIMIT);
  }

  if (selected.length === 0) {
    toastStore.warning(errors[0] || '没有可上传的音频文件');
    return;
  }
  if (errors.length > 0) {
    toastStore.warning(`${errors.length} 个文件被跳过（超出大小限制或格式不支持）`);
  }

  clearPickedUploadFiles();
  items.value = selected.map((file) => registerPickedFile(file, mode));
...
const handlePickerChange = async (event: Event, mode: PickMode) => {
  ...
  picking.value = true;
  try {
    await handlePickedFiles(files, mode);
  } catch (error) {
...
const openFilePicker = (mode: PickMode) => {
  if (!userStore.isLoggedIn || picking.value) return;
...
const uploadSingleItem = async (
  ...
  try {
    const file = pickedFiles.get(item.path);
    if (!file) throw new Error('本地文件已失效，请重新选择');
    ...
    if (shouldAbort(run)) return;
    item.isSecondUpload = !(res as { uploadInfo?: { upload_id?: string } })?.uploadInfo?.upload_id;
    item.status = 'success';
...
const handlePickManual = () => {
  if (!userStore.isLoggedIn || picking.value) return;
  manualInputRef.value?.click();
};

/** 手动匹配模式下，文件选择完成后不自动开始上传，先进入搜索表单 */
const handleManualPickerChange = async (event: Event) => {
  const input = event.target as HTMLInputElement;
  const files = Array.from(input.files || []);
  input.value = '';
  if (files.length === 0) return;
  const file = files[0];
  if (!isUploadAudioName(file.name)) {
    toastStore.warning(`${file.name}: 不是支持的音频文件`);
    return;
  }
  if (file.size <= 0 || file.size > CLOUD_UPLOAD_MAX_SIZE) {
    toastStore.warning(
  ...
  clearPickedUploadFiles();
  const item = registerPickedFile(file, 'file');
  manualFile.value = { ...item, title: item.title || file.name.replace(/\.[^.]+$/, '') };
  ...
  >
    <template #title>
      <span class="flex items-center gap-2">
        <Icon :icon="cloudUploadIcon" width="18" height="18" class="text-primary" />
        上传到云盘
      </span>
    </template>
    ...
    <input
      ...
      hidden
      multiple
      :accept="CLOUD_UPLOAD_EXTENSIONS.map((ext) => `.${ext}`).join(',')"
      @change="handlePickerChange($event, 'file')"
    />
    <input
      ref="folderInputRef"
      type="file"
      hidden
      multiple
      v-bind="{ webkitdirectory: '' }"
      @change="handlePickerChange($event, 'folder')"
    />
    <input
      ref="manualInputRef"
      type="file"
      hidden
      :accept="CLOUD_UPLOAD_EXTENSIONS.map((ext) => `.${ext}`).join(',')"
      @change="handleManualPickerChange($event)"
    />

    <template v-if="step === 'pick'">
      <div class="cloud-upload-pick-body">
        <button class="cloud-upload-option" :disabled="picking" @click="openFilePicker('file')">
          <div class="cloud-upload-option-icon">
            <Icon :icon="uploadIcon" width="20" height="20" />
          </div>
        ...
        <button class="cloud-upload-option" :disabled="picking" @click="openFilePicker('folder')">
          <div class="cloud-upload-option-icon">
            <Icon :icon="folderOpenIcon" width="20" height="20" />
          </div>
          <div class="min-w-0 flex-1 text-left">
