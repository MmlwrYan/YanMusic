...
import { audioEffectSupport, type AudioEffectSupport } from '@/utils/audioEffectSupport';
import {
...
// 生命周期属于音效广场容器，而非单个分类面板；收起容器不会丢失下载状态和分页缓存。
export const useAudioEffectPlaza = () => {
  ...
  const getEffectSupport = (effect: CommunityAudioEffect): AudioEffectSupport => {
    ...
    // Yan 使用 libmpv（afir 卷积），无外部 DSP 引擎注册表，能力判定走内置引擎分支。
    const localSupport = downloaded && audioEffectSupport(downloaded);
  ...
  // 选中即生效：播放器 store 的 setting 订阅会把 IR 路径下发给 libmpv（afir 卷积），
  // 无需再调用播放器方法。
  const selectDownloadedEffect = (id: string): boolean => {
    const file = settings.impulseResponseFiles.find((item) => item.id === id);
    if (!file) return false;
    const support = audioEffectSupport(file);
    if (support.status !== 'supported') {
      toast.warning(support.reason);
      return false;
    }
    if (settings.impulseResponseEnabled && settings.selectedImpulseResponseId === id) return true;
    settings.setSelectedImpulseResponse(id);
    return true;
  };
  const applyEffect = (effect: CommunityAudioEffect) => {
    ...
    return selectDownloadedEffect(downloaded.id);
  };
