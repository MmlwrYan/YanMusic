...
const songName = computed(() => playback.value?.title || 'YanMusic');
...
const renderLyricLines = computed<RenderLine[]>(() => {
  ...
  // 无歌词
  if (!lines.length) {
    if (!songName.value) return placeholder('YanMusic Desktop Lyric');
    return placeholder('YanMusic - 听你想听');
  }
  // 索引小于 0，显示歌曲名称
  if (idx < 0) {
...
<template>
  <div
    ...
    data-yan-lyric-host="desktop"
    :data-yan-lyric-layout="lyricLayout"
    :data-yan-lyric-effect-count="lyricEffectSummary.count"
    :data-yan-lyric-effect-decorator="lyricEffectSummary.hasDecorator ? 'true' : 'false'"
  >
    <!-- 顶部工具栏 -->
    <div class="header">
    ...
    <TransitionGroup
      ...
      data-yan-lyric-scroller="desktop"
      :data-yan-lyric-current-index="currentIndex"
      :data-yan-lyric-scroll-index="currentIndex"
    >
      <div
        ...
        data-yan-lyric-row
        :data-yan-lyric-index="line.index"
        :data-yan-lyric-current="line.active ? 'true' : 'false'"
        :data-yan-lyric-distance="line.index - currentIndex"
        :data-yan-lyric-abs-distance="Math.abs(line.index - currentIndex)"
        :data-yan-lyric-line-start-ms="
...
.desktop-lyric[data-yan-lyric-layout='vertical'] {
  padding: 10px;
}
...
.desktop-lyric[data-yan-lyric-layout='vertical'] .header {
  position: absolute;
  top: 10px;
...
.desktop-lyric[data-yan-lyric-layout='vertical'] .header-left,
.desktop-lyric[data-yan-lyric-layout='vertical'] .header-center,
.desktop-lyric[data-yan-lyric-layout='vertical'] .header-right {
  flex: 0 0 auto;
  display: flex;
  flex-direction: column;
...
.desktop-lyric[data-yan-lyric-layout='vertical'] .song-name {
  display: none;
}

.desktop-lyric[data-yan-lyric-layout='vertical'] .menu-btn {
  width: 30px;
  height: 30px;
  padding: 4px;
}

.desktop-lyric[data-yan-lyric-layout='vertical'] .offset-controls {
  flex-direction: column;
  gap: 6px;
}

.desktop-lyric[data-yan-lyric-layout='vertical'] .tran-group {
  flex-direction: column;
  gap: 0;
}

.desktop-lyric[data-yan-lyric-layout='vertical'] .tran-group .menu-btn {
  border-radius: 0;
}

.desktop-lyric[data-yan-lyric-layout='vertical'] .tran-group .menu-btn:first-child {
  border-radius: 8px 8px 0 0;
}

.desktop-lyric[data-yan-lyric-layout='vertical'] .tran-group .menu-btn:last-child {
  border-radius: 0 0 8px 8px;
}

.desktop-lyric[data-yan-lyric-layout='vertical'] .tran-group .menu-btn:only-child {
  border-radius: 8px;
}

.desktop-lyric[data-yan-lyric-layout='vertical'] .tran-group .menu-btn + .menu-btn {
  border-left: 0;
