...
2. **安装依赖**
   ...
   ```bash
   Error: ENOENT: no such file or directory, open '/home/xxx/Projects/Work/yanmusic/node_modules/electron/path.txt'
   ```
...
3. **编译 Rust 原生模块**
   ...
   ```bash
   Error: Cannot find module '/home/myname/yanmusic/native/yan-mpv-player/yan-mpv-player.node'
   [error] [MpvController] Failed to load yan-mpv-player addon
   ```

   需要手动编译 Rust 原生模块，因为 `*.node` 文件在 `.gitignore` 中被排除。推荐使用各 addon 自带的 napi-rs 构建脚本生成平台对应的 `.node`：

   ```bash
   cd native/yan-mpv-player
   npm install
   npm run build

   cd ../yan-media-controls
   npm install
   npm run build
   ...
   `yan-mpv-player` 集成了实时频谱分析功能，可选地支持系统音频捕获：Windows 使用 WASAPI loopback，Linux 使用 ALSA monitor，macOS 使用 ScreenCaptureKit。macOS 首次使用系统级频谱捕获时，需要在”系统设置 -> 隐私与安全性 -> 屏幕与系统音频录制”中授权 yanmusic；开发模式下也可能需要授权 Terminal、Electron 或当前启动进程。
...
插件声明 `capabilities.kugouVerification: true` 后，可以把接口返回的 `ssaCode` / `eventId` 交给 `ctx.kugouVerification.request()`；yanmusic 会复用主程序的安全验证弹窗，验证通过后插件再重试原请求。

```js
export async function activate(ctx) {
  const server = await ctx.webServer.listen(() => ({
    headers: { 'content-type': 'text/html; charset=utf-8' },
    body: '<!doctype html><title>yanmusic</title><h1>yanmusic Plugin Page</h1>',
  }));
